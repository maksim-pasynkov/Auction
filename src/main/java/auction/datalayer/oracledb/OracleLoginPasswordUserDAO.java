...
public class OracleLoginPasswordUserDAO implements LoginPasswordUserDAO {
	...
	public OracleLoginPasswordUserDAO(Connection connection) {
		this.connection = connection;
	}

	@Override
	public List<User> findUser(String login, String pass) {
		List<User> users = new ArrayList<>();
		PreparedStatement statement = null;
		try {
			statement = this.connection.prepareStatement(
					QueryManager.getProperty("sql.select.login.password.users"));
			...
			if (users.size() != 0) {
				statement = this.connection.prepareStatement(
						QueryManager.getProperty("sql.update.user.active"));
				statement.setInt(ID_INDEX, users.get(0).getId());
