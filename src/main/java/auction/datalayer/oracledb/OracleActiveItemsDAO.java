...
public class OracleActiveItemsDAO implements ActiveItemsDAO {
	...
	public OracleActiveItemsDAO(Connection connection) {
		this.connection = connection;
	}

	@Override
	public List<Item> findActiveItems(int userId) {
		List<Item> activeItems = new ArrayList<>();
		PreparedStatement statement = null;
		try {
			statement = this.connection.prepareStatement(
					QueryManager.getProperty("sql.select.active.items"));
