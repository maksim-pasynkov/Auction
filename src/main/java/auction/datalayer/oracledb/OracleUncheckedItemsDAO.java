...
public class OracleUncheckedItemsDAO implements UncheckedItemsDAO {
	...
	public OracleUncheckedItemsDAO(Connection connection) {
		this.connection = connection;
	}

	@Override
	public List<Item> findUncheckedItems() {
		List<Item> uncheckedItems = new ArrayList<>();
		PreparedStatement statement = null;
		try {
			statement = this.connection.prepareStatement(
					QueryManager.getProperty("sql.select.unchecked.items"));
			System.out.println(statement.getFetchSize()); 
			ResultSet resultSet = statement.executeQuery();
			while (resultSet.next()) {
				Item item = new Item();
				//item = (Item) resultSet.getObject(resultSet.getRow());
				//activeItems = (List<Item>) resultSet;
